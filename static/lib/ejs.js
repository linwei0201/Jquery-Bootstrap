...
;+function() {
    var fnReg = /\/\*([\s\S]*?)\*\//;
    $.getFrag = function(fn) {
        return (fn.toString().match(fnReg) || ['',''])[1] || '';
    }
}();
...
$.util.extend = function() {
    ...
    $.each(params, function(ka, va) {
        for(var k in va) {
            if(k in origin) {

            } else {
                if(typeof va[k] === 'function') {
                    origin[k] = va[k];
                } else {
                    origin[k] = deepClone(va[k]);
                }
            }
        }
    });
...
;+function() {
    ...
    ;!function() {
        for(var attr in escapeMap) {
            if(escapeMap.hasOwnProperty(attr)) {
                unescapeMap[escapeMap[attr]] = attr;
            }
        }
    }();
        ...
        var getObjKeys = function(obj) {
            ...
            for(var attr in obj) {
                if(obj.hasOwnProperty(attr)) {
                    keysRetArray.push(attr);
                }
            }
            return keysRetArray;
        };

        var source = '(?:' + getObjKeys(map).join('|') + ')';
    ...
    $.util.escapeHTML   = createEscaper(escapeMap);
    $.util.unescapeHTML = createEscaper(unescapeMap);
